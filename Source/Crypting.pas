...
type
  TFormCrypt = class
    function LanceDecodage(txt, Password: string):string;
    function LanceCodage(txt, Password: string):string;
...
function TFormCrypt.LanceCodage(txt, Password: string):string;
var
	Buf : PChar;
        Bufkey : Pchar;
        keysize : Integer;
...
begin
        Size := StrLen(pchar(txt));
        if (Size=0) then exit;
        keysize := StrLen(pchar(Password));
        if (keysize=0) then exit;
        GetMem(buf, Size+1);
        buf := pchar(txt);
        GetMem(Bufkey,keysize+1);
        Bufkey := pchar(Password);
	 ...
	 crypt(Bufkey, buf, Size);
         SetLength(result, length(Cry));
         result := Cry;
        end;
end;


function TFormCrypt.LanceDecodage(txt, Password: string):string;
var
	Buf : PChar;
        Bufkey : Pchar;
        keysize : Integer;
